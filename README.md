...
`src/pipeline/report.py` from the last pipeline run (seed `20240613`, run id
`83db588d6f30def2`).
...
flowchart TB
    subgraph gen["Data generation — deterministic, seeded"]
        G1["Synthetic watchlist<br/>5,000 SDN-style entities<br/>4 scripts, transliteration noise"]
        G2["Customer master<br/>50,000 customers"]
        G3["Transaction feed<br/>500,000 transactions / 12 months"]
    ...
    subgraph screen["Sanctions screening"]
        ...
        S4["Blocking index<br/>98.627% pair reduction"]
    ...
    subgraph tm["Transaction monitoring"]
        ...
        T2["Customer-month features<br/>54 features, label-free"]
        T3["Grouped split<br/>then fit baselines"]
        T4["LightGBM / XGBoost<br/>+ Isolation Forest"]
        T5["ATL/BTL sweep<br/>48 settings evaluated"]
    end

    subgraph gov["Governance"]
        V1["10 data-quality controls"]
        V2["Hash-chained audit log<br/>16,382 records"]
...
| | Result |
|---|---|
| Sanctions screening recall (planted hits) | **0.9071** (127 of 140) |
| Screening false-positive ratio | **16.5 : 1** at threshold 0.86 |
| Blocking pair-space reduction | **98.627%** (482,250,000 → 6,620,807 candidate pairs) |
| Blocking recall (planted hits retained) | **0.9929** |
| Best triage model | **xgboost**, ROC-AUC **0.9153**, PR-AUC 0.0802 |
| Model FP ratio at 0.851 recall | **31.0 : 1** — precision 0.0313 on 6,047 alerts |
| Rule-set recall (all 8 scenarios) | **0.4331** |
| Data-quality controls passing | **10 / 10** |
...
Deterministically generated from seed `20240613`. The same seed produces
byte-identical outputs; this is enforced in CI, not merely asserted.

| | |
|---|---|
| Watchlist entities | 5,000 |
| Customers | 50,000 |
| Transactions | 500,000 |
| Scored customer-months | 205,014 |
| True anomaly cells | 822 (0.401%) |
| Near-miss cells | 6,170 (3.01%) |
| Positives with no observable typology | 74 |
...
erDiagram
    ...
    CUSTOMERS {
        ...
        varchar national_id
        varchar occupation
        varchar behaviour_segment
        varchar kyc_risk_rating
        timestamp account_open_date
        boolean is_pep
        varchar watchlist_true_match_uid
        boolean is_screening_near_miss
    }
    TRANSACTIONS {
        varchar txn_id PK
        varchar customer_id
    ...
    LABELS {
        varchar customer_id PK
        varchar period PK
        boolean is_true_anomaly
        varchar typology
        varchar intensity
        boolean is_near_miss
    }
    CUSTOMERS ||--o{ TRANSACTIONS : customer_id
    CUSTOMERS ||--o{ LABELS : customer_id
```
...
3. **Some positives are rendered as nothing.** 74 labelled
...
|---|---|---|
| Anomaly vs. near miss (within injected cells) | **0.4593** | indistinguishable — the two populations are drawn from one renderer |
| Injected vs. ordinary cells | 0.9663 | legitimate signal: a typology should not look like a salary account |
| Mean score percentile, invisible positives | 0.531 | mid-pack, as a positive carrying no observable pattern must be |
| Mean score percentile, near-miss cells | 0.955 | ranked alongside true anomalies, which is why precision stays low |
...
Screening 50,000 customers against 9,645 reference names
— 5,000 entities plus every a.k.a. they carry, since the alias is
frequently the form a payment message actually contains — is
482,250,000 pairs. Blocking indexes those names under keys that any true
variant of them would also produce.

| | |
|---|---|
| Reference names indexed | 9,645 (4,645 of them aliases) |
| Full pair space | 482,250,000 |
| Candidate pairs after blocking | 6,620,807 |
| **Reduction** | **98.627%** |
| Mean candidates per subject | 132.4 |
| Largest block | 231 |
| Fully scored pairs (after cascade) | 1,776,274 |
| **Blocking recall on planted hits** | **0.9929** |
...
At the production threshold of 0.86:

| | |
|---|---|
| Planted true matches | 140 |
| Detected | 127 (**recall 0.9071**) |
| Customers alerted | 2,217 (4.43% of portfolio) |
| Precision | 0.0573 |
| False-positive ratio | 16.5 : 1 |
| Planted near misses alerted | 121 of 1,600 |
...
|---|---|---|---|---|
| R01_STRUCTURING | 2,304 | 0.0564 | 0.1582 | 16.7 : 1 |
| R02_VELOCITY | 1,347 | 0.0468 | 0.0766 | 20.4 : 1 |
| R03_ROUND_DOLLAR | 2,163 | 0.0102 | 0.0268 | 97.3 : 1 |
| R04_DORMANT_REACTIVATION | 1,188 | 0.0236 | 0.0341 | 41.4 : 1 |
| R05_NEW_ACCOUNT_BURST | 480 | 0.0104 | 0.0061 | 95.0 : 1 |
| R06_COUNTERPARTY_CONCENTRATION | 1,358 | 0.0007 | 0.0012 | 1357.0 : 1 |
| R07_HIGH_RISK_CORRIDOR | 4,446 | 0.0319 | 0.1727 | 30.3 : 1 |
| R08_CASH_INTENSITY | 774 | 0.0685 | 0.0645 | 13.6 : 1 |

Union recall across all eight scenarios: **0.4331**, on
14,060 alert rows.
...
1. Build label-free base features (54 features).
...
| | |
|---|---|
| Train | 153,770 rows / 27,889 customers |
| Test | 51,244 rows / 9,297 customers |
...
|---|---|---|---|---|---|---|
| lightgbm | 0.8801 | 0.0598 | 0.851 | 0.0176 | 10,724 | 55.7 : 1 |
| xgboost | 0.9153 | 0.0802 | 0.851 | 0.0313 | 6,047 | 31.0 : 1 |
| isolation_forest | 0.8298 | 0.0370 | 0.851 | 0.0094 | 20,166 | 105.7 : 1 |

Out-of-time check (trailing months of the held-out customers): ROC-AUC
0.9613 on 13,485 rows. The grouped split answers "does this generalise
...
- **Supervised ceiling: 0.92.** Deliberately not 0.90. At 0.4%
...
- **Unsupervised ceiling: 0.93 on the Isolation Forest**, which
  scores 0.8298 here. This is the sharper test and the one that caught a
...
48 settings evaluated across 8 rules, against a baseline that
produces 14,060 alerts in total. Example — `R01_STRUCTURING`
sweeping `min_txn_count`:

| Threshold | Position | Precision | Recall | Alert volume | FP ratio |
|---|---|---|---|---|---|
| 2 | LOOSER | 0.0443 | 0.1606 | 2,978 | 21.6 : 1 |
| 3 | BASELINE | 0.0564 | 0.1582 | 2,304 | 16.7 : 1 |
| 4 | TIGHTER | 0.0750 | 0.1034 | 1,133 | 12.3 : 1 |
| 5 | TIGHTER | 0.0871 | 0.1022 | 964 | 10.5 : 1 |
| 6 | TIGHTER | 0.0941 | 0.1022 | 893 | 9.6 : 1 |
| 8 | TIGHTER | 0.1023 | 0.0645 | 518 | 8.8 : 1 |
...
|---|---|---|---|---|---|---|---|---|
| R01_STRUCTURING | min_txn_count | 3 | 3 | 2,304 | 2,304 | 0.1582 | 0.1582 |
| R02_VELOCITY | min_passthrough_ratio | 0.8 | 0.8 | 1,347 | 1,347 | 0.0766 | 0.0766 |
| R03_ROUND_DOLLAR | min_round_share | 0.55 | 0.55 | 2,163 | 2,163 | 0.0268 | 0.0268 |
| R04_DORMANT_REACTIVATION | min_reactivation_amount | 15000 | 15000 | 1,188 | 1,188 | 0.0341 | 0.0341 |
| R05_NEW_ACCOUNT_BURST | min_total_amount | 25000 | 25000 | 480 | 480 | 0.0061 | 0.0061 |
| R06_COUNTERPARTY_CONCENTRATION | min_concentration | 0.7 | 0.7 | 1,358 | 1,358 | 0.0012 | 0.0012 |
| R07_HIGH_RISK_CORRIDOR | min_corridor_share | 0.3 | 0.3 | 4,446 | 4,446 | 0.1727 | 0.1727 |
| R08_CASH_INTENSITY | min_cash_share | 0.65 | 0.65 | 774 | 774 | 0.0645 | 0.0645 |
...
|---|---|---|---|---|
| DQ01_CUSTOMER_PK_UNIQUE | uniqueness | BLOCKER | PASS | 0 |
| DQ02_TXN_PK_UNIQUE | uniqueness | BLOCKER | PASS | 0 |
| DQ03_TXN_CUSTOMER_FK | referential_integrity | BLOCKER | PASS | 0 |
| DQ04_MANDATORY_NULLS | completeness | BLOCKER | PASS | 0 |
| DQ05_AMOUNT_RANGE | validity | BLOCKER | PASS | 0 |
| DQ06_TXN_DATE_WINDOW | validity | CRITICAL | PASS | 0 |
| DQ07_CHANNEL_DOMAIN | validity | CRITICAL | PASS | 0 |
| DQ08_COUNTRY_CODE_FORMAT | conformity | CRITICAL | PASS | 0 |
| DQ09_WATCHLIST_NAME_PRESENT | completeness | BLOCKER | PASS | 0 |
| DQ10_ACCOUNT_OPEN_BEFORE_TXN | consistency | WARNING | PASS | 0 |

### Audit

16,382 immutable records, chained by hash (head
`516ebe018366054a…`). Each carries the rule version, an input *snapshot* rather
...
dataset from the seed each time, and hashes 24 artefacts. Any
